...
@section('content')
    <div class="flex justify-center">
        <div class="item-center bg-gray-200 mt-6 p-4 w-6/12 rounded-xl">
            ...
            <form action="{{ route('showReservation', $reservation) }}" method="POST" >

                @csrf
                <div>
                <label for="zone">Zona</label>
                <select name="zone" id="zone" value="bulve" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-blue-500 w-full rounded-md @error('zone') border-red-500 text-red text-sm  @enderror">
                        {{-- <option value="" id="zone_id" selected disabled>Pasirinkti zoną</option>
                    @foreach (auth()->user()->reservations as $zone)
